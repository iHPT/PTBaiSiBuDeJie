...
@interface PTCommentViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@property (nonatomic,strong) PTModuleDataTool *tool;
...
@implementation PTCommentViewController
...
//    PTLog(@"viewDidAppear  %@", NSStringFromCGRect(self.tableView.frame));
}
...
-(void)setupHeader {
    ...
    PTTopicCell *cell = [PTTopicCell cell];
...
//设置tableView
-(void)setTableView {
    // tableView显示时实际高度比频幕高度小44
    self.tableView.contentInset = UIEdgeInsetsMake(44, 0, 0, 0);
    [self.tableView registerNib:[UINib nibWithNibName:NSStringFromClass([PTCommentCell class]) bundle:nil] forCellReuseIdentifier:cellID];
//    PTLog(@"bounds  %@", NSStringFromCGRect(SCREEN_FRAME));
//    PTLog(@"setTableView  %@", NSStringFromCGRect(self.tableView.frame));
}
...
-(void)getMoreData {
    ...
    self.page += 1;
    PTCommentModel *comment = [self.lastestComments lastObject];
    [self.tool getCommentsWithID:self.topicFrame.topic.ID page:self.page lastcid:comment.ID block:^(id json, NSInteger total) {
        ...
        for (PTCommentModel *comment in json) {
 ...
 * 返回comment
 */
- (PTCommentModel *)commentInIndexPath:(NSIndexPath *)indexPath {
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    PTCommentModel *comment = [self commentInIndexPath:indexPath];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (menu.isMenuVisible) {
        ...
        //取出点的那一行
        PTCommentCell *cell = (PTCommentCell *)[tableView cellForRowAtIndexPath:indexPath];
        //成为第一响应者
        [cell becomeFirstResponder];
        
        PTCommentModel *comment = cell.comment;
        UIMenuItem *ding = [[UIMenuItem alloc]initWithTitle:[NSString stringWithFormat:@"顶[%ld]", (long)comment.like_count] action:@selector(ding:)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    PTCommentCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
...
- (PTModuleDataTool *)tool {
    if (_tool == nil) {
        _tool = [[PTModuleDataTool alloc]init];
    }
    return _tool;
}
