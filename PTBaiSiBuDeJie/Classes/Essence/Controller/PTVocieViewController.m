...
@interface PTVocieViewController()

@end

@implementation PTVocieViewController
...
-(void)getNewData {
    ...
    @weakify(self)
    [[PTModuleDataTool tool] getDataWithArrayType:TopicTypeVoice parameterA:self.parameterA block:^(NSArray *topicsArray, NSString *maxtime) {
        @strongify(self)
...
-(void)getMoreData {
    ...
    @weakify(self)
    [[PTModuleDataTool tool] getDataWithMaxtime:self.maxtime page:@(page) TopicType:TopicTypeVoice parameterA:self.parameterA block:^(NSArray *moreTopicsArray,NSString *maxtime) {
        @strongify(self)
