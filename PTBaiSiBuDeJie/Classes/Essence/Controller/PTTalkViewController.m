...
@interface PTTalkViewController ()

@end

@implementation PTTalkViewController
...
-(void)getNewData {
    ...
    @weakify(self)
    [[PTModuleDataTool tool] getDataWithArrayType:TopicTypeTalk parameterA:self.parameterA block:^(NSArray *topicsArray, NSString *maxtime) {
        @strongify(self)
...
-(void)getMoreData {
    ...
    @weakify(self)
    [[PTModuleDataTool tool] getDataWithMaxtime:self.maxtime page:@(page) TopicType:TopicTypeTalk parameterA:self.parameterA block:^(NSArray *moreTopicsArray,NSString *maxtime) {
        @strongify(self)
